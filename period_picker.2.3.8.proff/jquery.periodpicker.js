 ...
 */
/*global jQuery,document,window,moment,clearTimeout,setTimeout,alert*/
(function ($, window, document) {
	...
	function PeriodPicker(startinput, options) {
		...
		that.addRange([date.parseStr(that.startinput.val()), date.parseStr(that.endinput.val())]);
		...
		that.timer1 = setTimeout(function () {
			...
			if (that.period.length) {
				...
				that.picker.find('.period_picker_years_period').css({
					width: Math.floor((that.options.yearSizeInPixels / 365) * moment(that.period[1]).diff(that.period[0], 'day')) + 'px',
					left: Math.floor((that.options.yearSizeInPixels / 365) * -moment([that.options.yearsPeriod[0], 0, 1]).diff(that.period[0], 'day'))
				});
	...
	PeriodPicker.prototype.init = function () {
		...
		that.picker.css({
			...
			width: this.options.cells[1] * that.options.monthWidthInPixels + (that.options.timepicker ? 87 * (!that.options.norange ? 2 : 1) : 0) + 50,
			...
			that.timepicker.each(function () {
				...
				if ($this.length && !$this.data('timepicker') && $.fn.TimePicker !== undefined) {
					...
					$this.on('change', function () {
						that.picker.find('.period_picker_submit_dates .period_picker_time input').eq(index).val(this.value).trigger('change');
					});
				}
			});
	...
	PeriodPicker.prototype.show = function () {
		...
		if (this.options.fullsize) {
			...
			if (!this.picker.hasClass('xdsoft_i_moved')) {
				...
				this.picker.css({
					left: offset.left,
					top: offset.top + this.button.height()
				});
			}
		}
		this.regenerate();
